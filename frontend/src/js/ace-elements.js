...
(function ($, undefined) {
    let multiplible = 'multiple' in document.createElement('INPUT');
    let hasFileList = 'FileList' in window;//file list enabled in modern browsers
    let hasFileReader = 'FileReader' in window;

    let Ace_File_Input = function (element, settings) {
        let self = this;
    ...
    Ace_File_Input.prototype.apply_settings = function () {
        let self = this;
        let remove_btn = !!this.settings.icon_remove;
    ...
    Ace_File_Input.prototype.show_file_list = function ($files) {
        let files = typeof $files === "undefined" ? this.$element.data('ace_input_files') : $files;
        ...
        for (let i = 0; i < files.length; i++) {
            let filename = typeof files[i] === "string" ? files[i] : $.trim(files[i].name);
            ...
            if (!this.well_style) this.$label.find('span').attr({'data-title': filename}).find('[class*="icon-"]').attr('class', fileIcon);
                ...
                let type = $.trim(files[i].type);
                let can_preview = hasFileReader && this.settings.thumbnail
                    &&
    ...
    let enable_drop_functionality = function () {
        let self = this;
        let dropbox = this.element.parentNode;
    ...
    let handle_on_change = function () {
        ...
        //user can return a modified File Array as result
        let files = !hasFileList ? null ://for old IE, etc
        ...
        if (files && files.length > 0) {//html5
            ...
            let name = $.trim(this.element.value);
    ...
    let preview_image = function (file) {
        let self = this;
        let $span = self.$label.find('span:last');//it should be out of onload, otherwise all onloads may target the same span because of delays

        let deferred = new $.Deferred;
        let reader = new FileReader();
        reader.onload = function (e) {
            $span.prepend("<img class='middle' style='display:none;' />");
            let img = $span.find('img:last').get(0);
            ...
            $(img).one('load', function () {
                ...
                let thumb = get_thumbnail(img, size, file.type);
                if (thumb == null) {
    ...
    let get_thumbnail = function (img, size, type) {
    ...
    $.fn.ace_file_input = function (option, value) {
        ...
        let $set = this.each(function () {
            let $this = $(this);
            let data = $this.data('ace_file_input');
...
(function ($, undefined) {
    $.fn.ace_spinner = function (options) {
        ...
        this.each(function () {
            let icon_up = options.icon_up || 'icon-chevron-up';
            let icon_down = options.icon_down || 'icon-chevron-down';

            let btn_up_class = options.btn_up_class || '';
            let btn_down_class = options.btn_down_class || '';

            let max = options.max || 999;
            max = ('' + max).length;
            let $parent_div =
...
(function ($, undefined) {
    $.fn.ace_wizard = function (options) {
        ...
        this.each(function () {
            ...
            let buttons = $this.siblings('.wizard-actions').eq(0);
            let $wizard = $this.data('wizard');
...
(function ($, undefined) {
    $.fn.ace_wysiwyg = function ($options, undefined) {
        ...
        this.each(function () {
            let toolbar = ' <div class="wysiwyg-toolbar btn-toolbar center"> <div class="btn-group"> ';

            for (let tb in toolbar_buttons) if (toolbar_buttons.hasOwnProperty(tb)) {
                let button = toolbar_buttons[tb];
                if (button === null) {
                ...
                let className = "className" in button ? button.className : '';
                switch (button.name) {
                    case 'font':
                    ...
                    case 'fontSize':
                        toolbar += ' <a class="btn btn-small ' + className + ' dropdown-toggle" data-toggle="dropdown" title="' + button.title + '"><i class="' + button.icon + '"></i>&nbsp;<i class="icon-angle-down icon-on-right"></i></a> ';
                        toolbar += ' <ul class="dropdown-menu dropdown-light"> ';
                        for (let tsize in button.values){
                    ...
                    case 'backColor':
                        ...
                        for (let color in button.values){
