!function (b) {
    var a = function (k, s, i) {
        ...
        this.separator = " - ";
        this.locale = {applyLabel: "Apply", cancelLabel: "Cancel", fromLabel: "From", toLabel: "To", weekLabel: "W", customRangeLabel: "Custom Range", daysOfWeek: moment.weekdaysMin(), monthNames: moment.monthsShort(), firstDay: 0};
        this.cb = function () {
        };
        this.element = b(k);
    ...
    a.prototype = {constructor: a, mousedown: function (c) {
        ...
        if (f.isSame(i) || f.isBefore(i)) {
            ...
            if (f.isAfter(i)) {
                b(j.target).addClass("active");
                this.startDate = f;
                this.endDate = moment(f).add(1,"day").startOf("day")
            }
        }
        this.leftCalendar.month.month(this.startDate.month()).year(this.startDate.year());
        this.rightCalendar.month.month(this.endDate.month()).year(this.endDate.year());
        ...
        var c = moment([q, o, 1]);
        var m = moment(c).subtract(1, "month").month();
        var l = moment(c).subtract(1, "month").year();
        ...
        for (var j = 0, d = 0, s = 0; j < 42; j++, d++, f = moment(f).add(1, "day")) {
            if (j > 0 && d % 7 == 0) {
                d = 0;
                s++
            }
