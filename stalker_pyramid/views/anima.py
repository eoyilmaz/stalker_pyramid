...
from stalker_pyramid.views import (get_logged_in_user, PermissionChecker,
                                   get_multi_integer, local_to_utc)
...
def add_related_assets(request):
    ...
    if entity.type.name == 'Scene':
        ...
        shots_folder = Task.query\
            ...
            .filter(Task.parent == scene).first()
...
def find_asset_task_by_type(asset, type_name):
    if type_name == 'Rig':
        ...
        animation_test_type = Type.query\
            .filter_by(name='Animation Test')\
            .first()
        animation_bible_type = Type.query\
            .filter_by(name='Animation Bible')\
            .first()

        animation_bible = Task.query\
            .filter(Task.parent == asset)\
            .filter(Task.type == animation_bible_type)\
            .first()

        if animation_bible:
            return animation_bible
        else:
            animation_test = Task.query\
                .filter(Task.parent == asset)\
                .filter(Task.type == animation_test_type)\
                .first()
            if animation_test:
                return animation_test
            else:
                rig_folder = Task.query\
                    .filter(Task.name == 'Rig')\
                    .filter(Task.parent == asset)\
                    .first()
                if rig_folder:
                    main_rig = Task.query\
                        .filter(Task.name == 'Main')\
                        .filter(Task.parent == rig_folder)\
                        .first()
                    if main_rig:
                        return main_rig
                    else:
                        hires_rig = Task.query\
                            .filter(Task.name == 'Hires')\
                            .filter(Task.parent == rig_folder)\
                            .first()
                        if hires_rig:
                            return hires_rig
...
def update_shot_task_dependecies(action, shot, task_name, dependencies, user, u_now):
    ...
    if task.status.code not in ['WFD', 'RTS']:
        return '%s: %s status is %s, task can not change!!!' % (
            shot.name, task.name, task.status.code
        )
