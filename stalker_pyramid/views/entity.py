...
def get_entity_events(request):
    ...
    if not multi_permission_checker(
        ...
        raise HTTPForbidden(headers=request.session)
...
group by
    -- date_trunc('week', results.start),
    results.resource_id,
    results.scheduled_resource_id,
    -- results.scene_name
    results.sequence_name
order by
    sequence_name, resource_ids
"""
    ...
    sql_query = sql_query % {
        'resource_where_conditions': resource_where_conditions,
        'task_type': task_type,
